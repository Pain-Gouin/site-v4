...
def check_checkout_status(checkout_intent_id):
    checkout = HelloAssoCheckout.objects.filter(checkout_intent_id=checkout_intent_id).first()
    if checkout is None:
        return
    
    if checkout.status in (HelloAssoCheckout.HelloAssoCheckoutStatusChoices.INITIATED, HelloAssoCheckout.HelloAssoCheckoutStatusChoices.PENDING):
...
def helloasso_payment_notification(payement, metadata=None):
    with transaction.atomic():
        if HelloAssoCheckout.objects.filter(payement_id=payement["id"]).exists():
            ...
            if checkout.status == HelloAssoCheckout.HelloAssoCheckoutStatusChoices.AUTHORIZED and payement["state"] == "authorized":
